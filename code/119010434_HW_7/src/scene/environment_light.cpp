...
namespace CGL { namespace SceneObjects {
  ...
  void EnvironmentLight::init() {
    ...
    for (int i = 1; i < h; i++)
    {
      marginal_y[i] = marginal_y[i] + marginal_y[i - 1];
    }
    for (int j = 0; j < h; j++)
    {
      for (int i = 1; i < w; i++)
      {
       conds_y[w * j + i] = conds_y[w * j + i] + conds_y[w * j + i - 1]; 
      }
    }
    if (true)
  ...
  Vector3D EnvironmentLight::sample_L(const Vector3D p, Vector3D* wi,
    ...
    //importance
    Vector2D uniform_2d_sample = sampler_uniform2d.get_sample();
    long y = std::upper_bound(marginal_y, marginal_y + envMap->h, uniform_2d_sample.y) - marginal_y;
    long x = std::upper_bound(conds_y + y * envMap->w, conds_y + (y + 1) * envMap->w, uniform_2d_sample.x) - (conds_y + y * envMap->w);
    Vector2D xy(x, y);
    Vector2D theta_phi = xy_to_theta_phi(xy);

    *wi = theta_phi_to_dir(theta_phi);
    *distToLight = INF_D;
    *pdf = pdf_envmap[x + envMap->w * y] * envMap->w * envMap->h / (2 * PI * PI * sin(theta_phi.x));
    return envMap->data[x + envMap->w * y];
  ...
  Vector3D EnvironmentLight::sample_dir(const Ray& r) const {
    ...
    Vector2D bilinear_interpolation = theta_phi_to_xy(dir_to_theta_phi(r.d));
    return bilerp(bilinear_interpolation);
  }
